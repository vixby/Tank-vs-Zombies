...
class PlayerTank:
    def __init__(self, position, color, angle=90):
        self.position = Point(position[0], position[1])
        self.color = color
        self.health = 10
        self.angle = angle
        self.speed = Vector(cos(radians(self.angle)),
                            sin(radians(self.angle)))
        self.turning_left = False
        self.turning_right = False
        self.turning_left_break = False
        self.turning_right_break = False
        self.going_up = False
    ...
    def fire(self):
        ...
        if (len(self.ammo) < self.max_ammo):
            new_projectile = Projectile(
                Point(self.position.x + dx, self.position.y + dy), self.turret.angle)
    ...
    def update_position(self):
        ...
        if self.turning_left and not self.turning_left_break:
            self.angle += angular_speed * delta_time

        if self.turning_right and not self.turning_right_break:
            self.angle -= angular_speed * delta_time
